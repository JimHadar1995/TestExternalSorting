using BigSortingAlgorithm.Internal;
using System;
using System.Linq;
...
namespace BigSortingAlgorithm
{
    public class SortAlgorithm
    {
        ...
        public SortAlgorithm(string sourceFilePath)
        {
            ...
            _outputTargetFileSet = new TargetFileSet(2) { new TargetFile("tb1"), new TargetFile("tb2") };
            _inputTargetFileSet = new TargetFileSet(2) { new TargetFile("ta1"), new TargetFile("ta2") };
        }
        ...
        public void Run()
        {
            ...
            using (var writer = new Writer(sorter))
            {
                ...
                Console.WriteLine($"{string.Join(", ", GetOutputTargetFileSet().Select(file => file.TargetFilePath))}: Общее количество прочитанных строк: {writtenCount}");
            }
            ...
            while (GetOutputTargetFileSet().Sum(file => file.RunsCount) > 1)
            {
                ...
                using (var writer = new Writer(mrg))
                {
                    ...
                    var writtenCount = writer.Write();
                    Console.WriteLine($"Общее количество прочитанных строк: {writtenCount}");
                }
            }
